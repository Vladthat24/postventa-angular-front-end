...
import { WarehouseByIdResponse, WarehouseResponse } from "../models/warehouse-response.interface";
...
export class WarehouseService {
  ...
  GetAll(
    ...
    const requestUrl = `${env.api}${endpoint.LIST_WAREHOUSE
  ...
  warehouseRegister(warehouse: WarehouseRequest): Observable<BaseResponse> {
    ...
    return this._http.post<BaseResponse>(requestUrl, warehouse).pipe(
      map((resp: BaseResponse) => {
        return resp;
      })
    );
  ...
  warehouseEdit(warehouseId: number, warehouse: WarehouseRequest): Observable<BaseResponse> {
